...
# ------------------------------------------------------------------------------
#
# run_install_full_docker.ksh: SBSDB - update the schemas.
#
# ------------------------------------------------------------------------------

docker start sbsdb

run_install_sbsdb_schema_update=$(docker inspect --format="{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}" sbsdb)

printf "Enter Database IP Address [default: $run_install_sbsdb_schema_update]: \n"; read DOCKER_IP; stty echo; printf "\n"

if [ -z $DOCKER_IP ]; then
    DOCKER_IP=$run_install_sbsdb_schema_update
fi
